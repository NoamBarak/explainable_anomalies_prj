...
class SubsetContainer:
    ...
    #     features = list(features)
    #     subset_mean = np.mean(self.subset[features], axis=0)
    #     raw_distance = np.linalg.norm(subset_mean - sample)
    #
    #     # Normalize: closer = higher score
    #     return 1 / (1 + raw_distance)

    def calc_euclidian_distance(self, sample, features):
        """
        Calculate normalized Euclidean distance (higher = more similar)
        """
        if sample is None:
            return None
        ...
        # Compute raw Euclidean distances for all rows
        distances = np.linalg.norm(subset - sample, axis=1)
        raw_distance = np.mean(distances)

        # Normalize: 1/(1+distance) - higher values mean more similar
        normalized_distance = 1 / (1 + raw_distance)
